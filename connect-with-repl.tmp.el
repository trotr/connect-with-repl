...
;; 			  (setq %connect-with-repl-running-yetp% nil))
;; 			line))
;; 		(connect-with-repl-send (scheme-proc) "(+ 10 1)")
;; 		(while %connect-with-repl-running-yetp%
;; 		  (sleep-for 0 connect-with-repl-wait-time))
;; 		(mapconcat 'identity connect-with-repl-tmp-storage "\n")))
;; (cnw-with-mutex (+ 1 2))
;; (connect-with-repl (funcall input-func))

;;      (catch 'result
;; 	  (while running-yet?
;; 	    (unless (> timeout 0) (throw 'result "Time out: connect-with-repl-internal")) 
;; 	    (sleep-for nil connect-with-repl-wait-time)
;; 	    (decf timeout 100))
;; 	  (mapconcat 'identity (setq connect-with-repl-storage (nreverse storage)))))))))
